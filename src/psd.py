...
def calculate_psd(src):
    ...
    psd_outpath = os.path.join(os.path.dirname(src),'plot_PSD.png')
    ...
    bandpowers_outpath = os.path.join(os.path.dirname(src),'plot_Bandpowers.png')
    plot_time_series(
        bandpowers, 
        x_col='time', 
        y_col='power', 
    ...
    # Save bandpowers
    bandpowers.to_csv(os.path.join(os.path.dirname(src), 'BANDPOWERS.csv'), index=False)
